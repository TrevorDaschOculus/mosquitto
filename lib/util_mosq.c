...
uint16_t mosquitto__mid_generate(struct mosquitto *mosq)
{
	...
	mosquitto_mutex__lock(&mosq->mid_mutex);
	mosq->last_mid++;
	if(mosq->last_mid == 0) mosq->last_mid++;
	mid = mosq->last_mid;
	mosquitto_mutex__unlock(&mosq->mid_mutex);
...
{
	mosquitto_mutex__lock(&mosq->state_mutex);
#ifdef WITH_BROKER
	...
	mosquitto_mutex__unlock(&mosq->state_mutex);
